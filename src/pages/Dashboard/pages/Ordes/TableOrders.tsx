...
export const TableOrders = ({ data }: { data: Order[] }) => {
	...
	const columns = [
		...
		{ field: "status", headerName: "Status", width: 180,  renderCell: (params: any) => {
		const status = params.value.toLowerCase();
		let color = "inherit";

		if (status.includes("pendente")) color = "#FFA500"; // laranja
		else if (status.includes("processando")) color = "#007bff"; // azul
		else if (status.includes("em transporte")) color = "#17a2b8"; // roxo
		else if (status.includes("cancelado")) color = "#dc3545"; // vermelho
